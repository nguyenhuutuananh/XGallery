        ...
        <nav class="navbar navbar-dark bg-dark">
            <form class="form-inline" method="post" action="{{route('jav.search.view')}}">
                ...
                <select class="custom-select form-control input-sm mr-sm-2" id="sort-by" name="sort-by">
                    <option @if(request()->get('sort-by') == 'id') selected @endif value="id">ID</option>
                    <option @if(request()->get('sort-by') == 'release_date') selected @endif value="release_date">Release date</option>
                    <option @if(request()->get('sort-by') == 'is_downloadable') selected @endif value="is_downloadable">Downloadable</option>
                </select>
